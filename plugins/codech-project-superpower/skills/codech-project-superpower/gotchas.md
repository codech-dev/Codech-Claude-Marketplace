...
> **Scope tagging.** Phase 5 gotchas are grouped by stack so you can skip what doesn't apply.
> - **Universal** — applies to any production system regardless of language/framework
> - **Python / FastAPI stack** — fires only if your backend uses Python + FastAPI/SQLModel/SQLAlchemy
> - **Node / pnpm / TypeScript stack** — fires only if your frontend uses pnpm + Node + TypeScript
> - **Playwright (visual regression)** — fires only if your visual-regression tool is Playwright

### Universal — Postgres 5432 conflicts with native install

**Scope:** Universal (any project using Postgres in Docker for dev)
...
### Universal — JWT base64url payload needs padding before any base64 decoder

**Scope:** Universal (any client decoding JWTs without a library)

**Symptom:** "Is access token expired?" check randomly returns `true` for valid tokens, forcing unnecessary refresh requests. About 1 in 4 tokens trigger it.

**Cause:** JWT bodies are base64url-encoded WITHOUT trailing `=` padding. Most base64 decoders require input length to be a multiple of 4 (with `=` padding). When the unpadded body's length mod 4 is 0, decoding works; otherwise it fails or decodes garbage.

**Fix (JS example — same pattern in any language):**
...
In Python use `base64.urlsafe_b64decode` after padding; in Go use `base64.RawURLEncoding` (which handles no-padding natively). Use a JWT library when possible — it handles this.

---

### Python / FastAPI stack — bcrypt 5.x silently breaks passlib 1.7.4
...
### Python / FastAPI stack — Alembic `script.py.mako` doesn't import `sqlmodel`
...
### Python / FastAPI stack — email-validator 2.3 rejects `.local` TLDs
...
### Python / FastAPI stack — mypy strict + SQLAlchemy column typing explodes
...
### Playwright — `page.getByText("Submit")` matches substrings
...
### Playwright — CI runner OS must match Playwright baseline OS
...
### Node / pnpm stack — pnpm 11 needs Node 22.13+ (`node:sqlite`)
...
### Node / pnpm stack — `tsc --build` pollutes the source tree with `.js` files
